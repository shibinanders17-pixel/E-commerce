...
export default function AdminOrderDetails() {
  ...
  if (!order) {
    return <div className="p-6 text-center text-gray-400 text-lg">Order not found!</div>;
  }
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => navigate("/admin/orders")}
          className="px-4 py-2 bg-white border border-gray-200 text-gray-600
                     rounded-xl text-sm font-semibold hover:bg-gray-50 transition shadow-sm">
          ← Back to Orders
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-5">
        <div className="bg-orange-400 px-5 py-4 flex items-center justify-between">
          <div>
            <p className="text-white text-xs font-semibold opacity-80 uppercase tracking-wider">Order ID</p>
            <p className="text-white font-bold text-lg font-mono">#{order._id.slice(-8).toUpperCase()}</p>
          </div>
          <div className="text-right">
            <p className="text-white text-xs font-semibold opacity-80 uppercase tracking-wider mb-1">Update Status</p>
            <select
              value={order.status}
              onChange={(e) => handleStatusUpdate(e.target.value)}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold outline-none border-0 cursor-pointer
        ...
        <div className="p-5 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p className="text-sm font-semibold text-gray-800 mt-1">
              {new Date(order.purchaseDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Payment</p>
            <span className={`inline-flex items-center gap-1 mt-1 px-2 py-1 rounded-lg text-xs font-semibold
                             ${paymentColors[order.paymentMethod] || "bg-gray-100 text-gray-700"}`}>
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-gray-800 mt-1">{order.totalItems}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Total Amount</p>
            <p className="text-lg font-bold text-orange-600 mt-1">₹ {order.totalPrice?.toLocaleString("en-IN")}</p>
          </div>
        </div>

        {/* Delivery / Cancelled Info */}
            ...
            <div className="bg-blue-50 rounded-xl px-4 py-2.5 flex items-center gap-2">
              ...
              <p className="text-xs font-semibold text-blue-700">
                {order.status?.includes("Delivered")
                  ...
                  : `Estimated Delivery: ${getEstimatedDelivery(order.purchaseDate)}`}
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-5">
        ...
        <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Customer Name</p>
            <p className="text-sm font-semibold text-gray-800 mt-1">{order.userName}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Email</p>
            <p className="text-sm font-semibold text-gray-800 mt-1">{order.userId?.username || "—"}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Phone</p>
            <p className="text-sm font-semibold text-gray-800 mt-1">{order.phone}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Pincode</p>
            <p className="text-sm font-semibold text-gray-800 mt-1">{order.pincode || "—"}</p>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-5 flex flex-col gap-4">
          {order.products?.map((product, index) => (
            <div key={index}
              ...
              <div className="w-16 h-16 bg-white rounded-xl flex items-center justify-center border border-gray-200 shrink-0">
                {product.image ? (
                  <img src={`http://localhost:5002${product.image}`} alt={product.name} className="w-12 h-12 object-contain" />
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-blue-500 mt-0.5 cursor-pointer hover:underline"
                  onClick={() => window.open(`/productsDet/${product.productId}`, "_blank")}>
                  View Product →
                </p>
              </div>
              <div className="text-right">
          ...
          <div className="flex justify-between items-center pt-4 border-t border-dashed border-gray-200">
            ...
            <p className="text-xl font-bold text-orange-600">₹ {order.totalPrice?.toLocaleString("en-IN")}</p>
          </div>
        </div>
      </div>
