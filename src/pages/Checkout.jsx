...
export default function Checkout() {
  ...
  const fetchSingleProduct = async () => {
    try {
    const res = await api.get(`/users/products/${id}`);
    const passedQty = location.state?.qty || 1; 
    setOrderProducts([{ ...res.data, qty: passedQty }]);
    } catch (error) {
      console.log(error);
    } finally {
      setFetching(false);
    ...
    <div className="min-h-screen bg-gray-100 py-6 px-4 md:px-10">
      ...
      <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                rows="3"
                className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm outline-none
                           focus:border-red-400 focus:ring-1 focus:ring-red-200 transition resize-none"
              />
              <input
                type="text"
                placeholder="Phone Number (10 digits)"
          ...
          {/* Wallet Section — wallet balance irundha mattum show aagum */}
          {walletBalance > 0 && (
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <div className="flex items-center gap-2 mb-4">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold text-gray-700">Use Wallet Balance</p>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 max-h-60 overflow-y-auto pr-1">
              {orderProducts.map((item) => (
                <div key={item._id} className="flex gap-3 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xs font-semibold text-gray-800 line-clamp-2">{item.name}</h3>
                    <p className="text-red-500 font-bold text-xs mt-0.5">
                      ₹ {item.price.toLocaleString("en-IN")}
                    </p>
                    {/* ✅ Single product checkout la qty control */}
                    {!isCartCheckout ? (
                      <div className="flex items-center gap-2 mt-1">
                        <button
                          onClick={() => setOrderProducts(prev =>
                          ...
                          className="w-6 h-6 rounded-full bg-gray-200
                                     ...
                                     font-bold text-sm flex items-center justify-center transition"
                        >
                          −
                        </button>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3 pb-2 border-b border-gray-100">
              Price Details
            </h3>
            <div className="space-y-2 text-sm text-gray-600">
              <div className="flex justify-between">
            ...
            {useWallet && walletDiscount > 0 && (
              <p className="text-xs text-green-600 mt-1 font-medium">
                🎉 ₹ {walletDiscount.toLocaleString("en-IN")} saved using wallet!
              </p>
            )}
            <button
              onClick={handlePlaceOrder}
